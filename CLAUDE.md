...
**Authentication.** JWT (HS256) issued by `apps/member` (POST `/api/members/login`). The gateway validates every request via `JwtAuthenticationFilter` (order = -100), then reads the full `UserContext` directly from Redis (`user:{id}`) and forwards it as a single Base64-encoded JSON header `X-User-Context`. No `X-User-Id`/`X-User-Role` are sent — services use `@AuthUser UserContext` (provided by `libs/shopping-context` via Spring auto-config) to receive the typed object. Public paths are listed under `jwt.public-paths` in gateway yml (currently `/api/members/login`, `/api/members/signup`, `/actuator/**`); CORS preflight (OPTIONS) bypasses the filter. Downstream services do NOT run Spring Security or call Redis — they only deserialize the header.

**Cache ownership** is intentionally split: `apps/member` is the **sole writer** (signup, login, future profile updates) and the gateway is **read-only**. There is no HTTP fallback from gateway to member-service — if the cache is missing, the gateway returns 401 (`USER_NOT_FOUND`) and the client must re-login. To make this safe, `user-context-cache.ttl-minutes` (1500m / 25h on member-service) is set longer than `jwt.expiration-hours` (24h), so a valid token always finds its cache entry. Cache miss only happens on Redis restart/flush or admin-forced eviction — both are intentional "force-logout" semantics.

Roles: `CUSTOMER`, `SELLER`, `ADMIN`. Self-signup is allowed only for CUSTOMER/SELLER; ADMIN is provisioned by `AdminSeeder` on first boot using `admin-seed.*` properties (default `admin@msa-shop.com` / `admin1234` — change in non-dev). The shared JWT `secret` lives in both `apps/member` and `infra/gateway` yml — must match.
