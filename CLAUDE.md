...
**Authentication.** JWT (HS256) issued by `apps/member` (POST `/api/members/login`). The gateway validates every request via `JwtAuthenticationFilter` (order = -100), then resolves the full `UserContext` (Redis cache → fallback `lb://member-service/api/members/{id}`) and forwards it as a single Base64-encoded JSON header `X-User-Context`. No `X-User-Id`/`X-User-Role` are sent — services use `@AuthUser UserContext` (provided by `libs/shopping-context` via Spring auto-config) to receive the typed object. Public paths are listed under `jwt.public-paths` in gateway yml (currently `/api/members/login`, `/api/members/signup`, `/actuator/**`); CORS preflight (OPTIONS) bypasses the filter. Downstream services do NOT run Spring Security or call Redis — they only deserialize the header. `apps/member` writes the cache (`user:{id}`, 30-min TTL by default) on signup/login; the gateway repopulates on cache miss. Roles: `CUSTOMER`, `SELLER`, `ADMIN`. Self-signup is allowed only for CUSTOMER/SELLER; ADMIN is provisioned by `AdminSeeder` on first boot using `admin-seed.*` properties (default `admin@msa-shop.com` / `admin1234` — change in non-dev). The shared JWT `secret` lives in both `apps/member` and `infra/gateway` yml — must match.
